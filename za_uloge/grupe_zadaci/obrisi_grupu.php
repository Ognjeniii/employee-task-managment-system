...
<body>
    <div class="form-container">
        <form action="../../logika/brisanje_grupe.php" method="post">
            <label>Unesite ID grupe koju zelite da obrisete:</label>
            <select name="id" class="grupe-select">
                <?php foreach($sveGrupe as $grupa) : ?>
                    <option value="<?= $grupa->id ?>"><?= $grupa->Naziv_grupe ?></option>
                <?php endforeach ?>
            </select>
            <input type="submit" class="dugme" value="OBRISI">
        </form>
    </div>
</body>
</html>